...
import { buildMemoPdf } from '@/lib/vc/report/buildMemoPdf';
...
export async function POST(request: Request) {
  try {
    ...
    const buffer = await buildMemoPdf(opportunityId);
    // Buffer is not a valid BodyInit under strict types
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': 'attachment; filename="vcbrain-memo.pdf"',
      },
    });
  } catch (err) {
